function preprocessing_BIDS (sub, nruns, basedir)
...
It also assumes you acquired a T1 analtomical image, at least 1 functional run saved as a 4D nifti file, and possibly a field map.
...
Adapted from a preprocessing pipeline by Carlos Gonzalez-Garcia and David Wisniewski at University of Gent, 
published originally at https://github.com/CCN-github/fMRI-preprocessing-SPM12 (downloaded in December 2021)
 %}
...
param.st.nslices = 17; % number of slices per volume
param.st.tr = 1.; %TR in sec
param.st.ta = 1.-(1./17); %time of acquisition for one slice, i.e. TR - (TR./nslices)
param.st.so = [0, 0.529412, 0.0588235, 0.588235, 0.117647, 0.647059, 0.176471, 0.705882, 0.235294, 0.764706, 0.294118, 0.823529, 0.352941, 0.882353, 0.411765, 0.941176, 0.470588]*1000; 
% slice acquisition order
param.st.refslice = max(param.st.so)/2; % reference slice, for fMRI usually the middle slice
% smoothing
param.smooth.fwhm = [3 3 3]; % define the smoothing kernel in mm
...
%dir basedir
%% RUN THE ANALYSIS
matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.files = cellstr(spm_select('FPList',[basedir subdir '/anat/'],'^sub.*.nii.gz$'));
%matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.outdir = {out_anat};
matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.keep = true;
spm_jobman('run', matlabbatch)
clear matlabbatch

matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.files = cellstr(spm_select('FPList',[basedir subdir '/func/'],'^sub.*.nii.gz$'));
%matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.outdir = {out_anat};
matlabbatch{1}.cfg_basicio.file_dir.file_ops.cfg_gunzip_files.keep = true;
spm_jobman('run', matlabbatch)
clear matlabbatch
...
if do_deface
    ...
    mkdir([derivativesdir subdir '/anat/'])
    destination = regexprep(fname, basedir, derivativesdir)
    movefile (fname, destination) ;
    fname=spm_select('FPList',[derivativesdir subdir '/anat/'],'^anon.*.nii$');
...
if do_fieldmap
        ...
        matlabbatch{1}.spm.tools.fieldmap.calculatevdm.subj.defaults.defaultsval.mflags.template = {'F:/Data/Programs Scripts/spm12/toolbox/FieldMap/T1.nii'}; % change the folder here
...
if do_realign_unwarp
    ...
    for run=1:nruns
        filter = ['^sub.*run-' num2str(run,'%.2d') '_bold.nii$'] ;
    ...
    mkdir([derivativesdir subdir '/func/'])
...
if do_segmentation == 1
    ...
    matlabbatch{1}.spm.spatial.preproc.tissue(1).tpm = {'spm12/tpm/TPM.nii,1'}; % change the folders here
    matlabbatch{1}.spm.spatial.preproc.tissue(1).ngaus = 1;
    matlabbatch{1}.spm.spatial.preproc.tissue(1).native = [1 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(1).warped = [0 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(2).tpm = {'spm12/tpm/TPM.nii,2'}; % and here
    matlabbatch{1}.spm.spatial.preproc.tissue(2).ngaus = 1;
    matlabbatch{1}.spm.spatial.preproc.tissue(2).native = [1 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(2).warped = [0 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(3).tpm = {'spm12/tpm/TPM.nii,3'}; % and here
    matlabbatch{1}.spm.spatial.preproc.tissue(3).ngaus = 2;
    matlabbatch{1}.spm.spatial.preproc.tissue(3).native = [1 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(3).warped = [0 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(4).tpm = {'spm12/tpm/TPM.nii,4'}; % and here
    matlabbatch{1}.spm.spatial.preproc.tissue(4).ngaus = 3;
    matlabbatch{1}.spm.spatial.preproc.tissue(4).native = [1 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(4).warped = [0 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(5).tpm = {'spm12/tpm/TPM.nii,5'}; % and here
    matlabbatch{1}.spm.spatial.preproc.tissue(5).ngaus = 4;
    matlabbatch{1}.spm.spatial.preproc.tissue(5).native = [1 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(5).warped = [0 0];
    matlabbatch{1}.spm.spatial.preproc.tissue(6).tpm = {'spm12/tpm/TPM.nii,6'}; % and here
...
if do_norm
    ...
    % Bounding box (bb): simply the default, so took it away
    %matlabbatch{1}.spm.spatial.normalise.write.woptions.bb = [[-78 -112 -70];[78 76 85]];
    % Choosing resolution
    matlabbatch{1}.spm.spatial.normalise.write.woptions.vox = [3 3 5];
    matlabbatch{1}.spm.spatial.normalise.write.woptions.interp = 4;
    matlabbatch{1}.spm.spatial.normalise.write.woptions.prefix = 'final_';
    spm_jobman('run', matlabbatch); 
end
